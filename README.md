...
- **Site Image Catalog** — select Kubeport-owned public GHCR Frappe/ERPNext runtime images from a DB-backed catalog when deploying supported bench charts. Users can also register their own pre-built images alongside the curated defaults; an `origin` flag distinguishes curated (`Kubeport`) rows from custom (`User`) rows so the daily sync never overwrites user entries. v1 supports public images on Frappe v16 and ERPNext-style chart values.
