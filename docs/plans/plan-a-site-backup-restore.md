# Plan A — Frappe Site Backup / Restore

## Intent

`Frappe Site` covers create, delete, and migrate. `delete_site_task` runs `bench drop-site
--no-backup --force`, which is genuinely irreversible: a misclick in the UI permanently destroys a
site's database and files. Plan A closes that gap by giving every `Frappe Site` a backup/restore
lifecycle that mirrors the patterns already proven for create/delete/migrate.

The cycle's goal is **no-data-loss operation**: an operator should be able to recover from a
mistaken `delete_site` (or a corrupt migration) within minutes by restoring the most recent
backup, without leaving the Kubeport UI.

Out of scope for this cycle (deferred to a follow-up): scheduled/automatic backups, object-store
backends (S3/GCS/Azure Blob), cross-cluster restore, retention policies beyond a manual
delete-old-backup button.

## Architectural fit

Backup/restore are cluster-mutating operations on a `Frappe Site`, so they follow the same rules
as the existing site lifecycle (`AGENTS.md` invariants, `CLAUDE.md` summary):
