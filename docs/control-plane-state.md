...
- For ERPNext/Frappe-style charts, Kubeport prepares deployable starter values by discovering the cluster's default StorageClass (the one annotated `storageclass.kubernetes.io/is-default-class: "true"`) when the user's values do not specify `persistence.worker.storageClass`. User-supplied storage classes always win. When the discovered class is `local-path` (or another known RWO-only provisioner), Kubeport renders `accessModes: ["ReadWriteOnce"]` when the chart-default RWX mode would not schedule on that backend. Discovery is read-only — the result is not persisted to MariaDB. If no default-class annotation exists and the user has not set the key, the load/deploy path fails fast with an actionable message instead of bubbling up the ERPNext chart's `required` template error.
- **Bundled MariaDB by default for Frappe charts.** Frappe/ERPNext charts ship with `dbHost` empty, so a freshly deployed release cannot create sites until the operator wires a MariaDB. Kubeport closes that gap: when the chart matches `is_frappe_site_chart()`, the install/upgrade worker also installs the upstream Bitnami MariaDB chart (pinned: `oci://registry-1.docker.io/bitnamicharts/mariadb` at version `25.1.1`) as a sibling release named `<release-name>-mariadb` in the same namespace, and renders `dbHost: <release-name>-mariadb` into the parent release's values so the chart resolves to the sibling Service. Uninstalling the parent uninstalls the sibling. The **Use External Database** checkbox on the Helm Release form opts out — when checked, no sibling release is installed and the operator must supply `dbHost` themselves through the raw `Values` YAML and provide root credentials on each `Frappe Site`. The bundled flow is also the basis of `Frappe Site._preflight_db_topology`, a synchronous Service/Secret lookup that refuses to enqueue a `bench new-site` Job when the bundled-MariaDB happy path is obviously broken (no `<release-name>-mariadb` Service in the namespace).
- **Structured ingress fields for Frappe charts.** The Helm Release form exposes four fields — `ingress_enabled`, `ingress_hostname`, `ingress_class_name`, `ingress_cluster_issuer` — that render the chart's `ingress.*` values when enabled. TLS is opt-in via cert-manager: when `ingress_cluster_issuer` is set, Kubeport emits the `cert-manager.io/cluster-issuer` annotation and a `tls` block referencing a per-release secret `<release-name>-tls`. Rendering is gated on `is_frappe_site_chart()`. The structured fields are authoritative for simple cases, but the user's raw YAML wins when the `ingress:` block is **advanced**, classified by `_has_advanced_ingress_override()`: more than one host, any path other than `/ ImplementationSpecific`, annotations beyond `cert-manager.io/cluster-issuer`, or any top-level key outside `{enabled, className, hosts, annotations, tls}`. Empty or chart-default `ingress` blocks fall through to the structured rendering. The four form fields participate in `calculate_release_spec_hash`, so toggling ingress without touching `values` still flips `pending_changes`.
- **Read-only ingress discovery hints.** While editing a Helm Release, the form populates non-persisted suggestions for `ingress_class_name` (the cluster's default `IngressClass` or the only one detected) and `ingress_cluster_issuer` (a ready cert-manager `ClusterIssuer`). When a LoadBalancer ingress controller has an external IP and no hostname is set, the form proposes `<release-name>.<ip>.nip.io` for local/dev clusters without a real DNS zone. The hints are pulled live, never persisted, and the corresponding fields stay editable when nothing is detected.
...
- **Lifecycle states**: `Draft → In Progress → Active | Failed`, plus `Active → Migrating → Active | Failed` and `Active|Failed → Deleting → [doc deleted] | Failed`. Backup re-uses the `In Progress` state — `Active → In Progress → Active | Failed` while a backup Job runs against the site — and a re-attempt after a failed create reuses the original arrow as `Failed → In Progress`. Cancellation rotates the operation token, marks the row `Failed`, and best-effort deletes the Job. Cancelling `Migrating` is confirmation-gated because killing `bench migrate` can leave MariaDB schema changes half-applied. The full set of observable site arrows is enumerated in `kubeport/tests/test_property_fsm_frappe_site.py::DOCUMENTED_SITE_ARROWS` and verified against random rule sequences by the property-based FSM test there.
...
- **Scheduled backups + retention.** `Frappe Site` carries an optional `backup_schedule` (five-field cron) and two retention bounds (`backup_retention_count`, `backup_retention_days`). The backup reconciliation tick (`reconcile_site_backups`, every 5 min) computes the next cron firing from `backup_schedule_last_run` (falling back to `creation`); when the next slot is in the past it advances the marker to `now()` then enqueues a single backup via the same `_enqueue_backup` helper used by the **Backup Now** button. Long downtime triggers exactly one catch-up backup per overdue slot, not a flood, because the marker advance bounds croniter's `get_next` base on the next tick. Retention pruning runs after the schedule pass and only trashes rows with status `Available`; `Failed` rows (diagnostic) and `In Progress`/`Restoring` rows (already guarded by `FrappeSiteBackup.on_trash`) are never auto-trashed. The existing `on_trash` hook propagates the deletion to the PVC archive, so retention shares its cleanup path with manual trash.
...
- `Frappe Site` covers create, delete, migrate, backup, restore, scheduled backups, and per-site retention. Object-store storage, cross-cluster restore, and restore-to-different-site-name remain out of scope.
...
2. **Backup depth**: scheduled backups and per-site retention shipped 2026-05-10. Remaining: object-store backends, encryption at rest, cross-cluster restore, and restore-to-different-site-name (the latter blocked on a richer storage-path model).
