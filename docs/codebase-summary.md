...
- Renders structured ingress (`ingress_enabled`, `ingress_hostname`, `ingress_class_name`, `ingress_cluster_issuer`) into `ingress.*` for Frappe charts. cert-manager TLS is opt-in via `ingress_cluster_issuer`. The user-supplied `values` YAML wins — a pre-existing `ingress` key disables the structured rendering and serves as the multi-host / advanced-config escape hatch. All four fields participate in the spec hash so toggling ingress flips `pending_changes`.
...
- **Scheduled backups + retention**: after the in-flight finalisation pass, `_run_scheduled_backups` enqueues one backup per `Active` site whose `backup_schedule` cron is overdue (computed from `backup_schedule_last_run`, advancing the marker before enqueueing so concurrent ticks observe the slot is taken). `_prune_backup_retention` then trashes `Available` rows beyond the per-site `backup_retention_count` and `backup_retention_days` caps, sharing the existing `FrappeSiteBackup.on_trash` archive cleanup path.
