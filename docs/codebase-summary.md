...
- Reads `helm get manifest` to enumerate Helm-owned resources and checks readiness for built-in workload kinds (`Deployment`, `StatefulSet`, `DaemonSet`, `Pod`, `Job`).
...
- Keeps observed per-resource rows ephemeral; only `status`, `helm_revision`, and short `helm_status_detail` are persisted on the Helm Release row.
...
- `install_or_upgrade_release` — idempotent `helm upgrade --install` with operation-token stale-job guards and shared health classification
- `uninstall_release` — `helm uninstall` with stale-job guard; Helm "release not found" is treated as successful cleanup and returns the row to `Draft`
...
- **Helm Releases**: queries `helm status` for all `Deployed`/`Degraded` releases, then applies the shared manifest/readiness classifier. It can recover `Degraded` rows to `Deployed`, flag unready workloads as `Degraded`, and mark pending/non-deployed Helm states as `Failed`.
